...
class VectorSearchManager:
    ...
    def _wait_for_index_ready(
        ...
        while time.time() - start < timeout:
            ...
            # Log full response on first attempt to help debug unexpected UNKNOWN states
            if attempt == 0:
            ...
            if index_status in terminal_failed:
                raise Exception(
                    f"Vector index '{self.index_name}' entered a terminal state: {index_status}"
                )
        ...
        raise TimeoutError(
            f"Vector index '{self.index_name}' did not become ONLINE within {timeout}s. "
            "Check the Databricks Vector Search UI for details."
        )

    def sync_index(self, ready_timeout: int = 600, poll_interval: int = 15) -> None:
        ...
        Args:
            ready_timeout: Max seconds to wait for the index to become ready (default 10 min)
        ...
        self._wait_for_index_ready(index, timeout=ready_timeout, poll_interval=poll_interval)
